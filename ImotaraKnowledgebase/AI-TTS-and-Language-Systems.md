...
- **Crisis resources** (`crisisResources.ts`) — country-specific emergency numbers and mental-health/suicide helplines for ~59 countries (India defaults: Tele-MANAS 14416, KIRAN 1800-599-0019, NIMHANS, Emergency 112). Country is detected client-side (`detectCountry.ts`: `Intl.DateTimeFormat` locale → `navigator.language` → IANA-timezone lookup), with a manual override in Settings; falls back to India if nothing resolves. (Not a Vercel geo header — corrected 2026-08-14.)
- **Crisis detection (client-side banner, 3-tier)** — a separate detector from the one below, English + ~9 Indian languages only. Tier 2 (immediate danger / suicidal-ideation) shows a non-dismissible red banner with a tappable helpline number; Tier 1 (hopelessness, worthlessness, "trapped") shows an amber banner; Tier 0 is normal flow. Mobile's equivalent is a dismissible card attached to the bot's reply, not a banner.
- **Crisis detection (server-side, `CRISIS_HINT_REGEX`)** — the detector that actually drives AI reply behavior: covers all **22** Imotara-supported languages (extended from 13 on 2026-08-14), used in `/api/chat-reply` (lowers reply temperature to 0.4, suppresses the Connect-referral mention, force-includes extended therapeutic guidance, adds a recall-boosted end-of-prompt crisis reminder) and in the rule-based fallback orchestrator. On detection, a **metadata-only** event (user, platform, language, timestamp — never message text) is logged to `crisis_events`, viewable at `/admin/crisis-events` (dashboard-only, no real-time alert — see Admin Guide §15).
