...
- **SSML express-as styles per language:** only voices with a style *confirmed present in Azure's live per-voice `StyleList`* get `<mstts:express-as>` — English (female `friendly`; male has no style support at all), Chinese (female `gentle`, male `chat`), Japanese (female `chat`; male none), French (both `cheerful`), Portuguese (female `calm`; male none). German, Spanish, all Indian-language voices, Arabic, and Russian have no confirmed style support in Azure's catalog and get plain unstyled speech. English also supports an emotion-aware style override (`sad`/`friendly`/`cheerful`/`excited`, chosen from the user's detected emotion) — implemented backend-side in `resolveStyle()` and, as of 2026-08-13, wired to the hands-free/auto-read TTS call sites on both platforms (`speakMessage()` on mobile, `autoSpeakText()` on web), each with its own local-emotion-vocabulary-to-canonical-vocabulary mapping (`mapUserEmotionForTTS()`/`mapDebugEmotionForTTS()`) so values like "stressed" or "angry" actually match `resolveStyle()`'s lookup instead of silently missing it. Manual (tap-to-read) playback does not pass emotion. Corrected 2026-08-13: the previous config had 5 of 12 entries requesting styles the voice didn't actually support (Azure silently ignores an unsupported style rather than erroring, so these were getting flat default delivery despite the config implying otherwise), and non-styled languages previously got an asymmetric `<prosody rate="-8%" pitch="+1%">` fallback wrapper that was inadvertently making speech sound slower/off-pitch, not more "considered" — that fallback has been removed; non-styled languages now get plain, unwrapped speech, same as English's own unstyled male voice always did.
