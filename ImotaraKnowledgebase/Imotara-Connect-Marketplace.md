...
> **⚠ UPDATE 2026-08-08 (corrects the 2026-07-26 entry below).** The 2026-07-26 retirement was incomplete: it removed the Wallet *tab's* top-up card (`connect/page.tsx`) but missed a second, separate wallet chip on `ConsultantCard.tsx` (every companion card + its profile modal) that stayed live and reachable — one real user completed a ₹1 top-up through it on 2026-07-26, the same day the tab-level removal shipped. Closed 2026-08-08: the `ConsultantCard.tsx` chip and its `WalletTopUpModal` are now deleted (not just unrendered), and `topup/create`/`topup/verify` are **hard-blocked** — both now return `410 Gone` unconditionally, they no longer function even if called directly. This is a change from the dormant-Stripe-backend pattern used elsewhere in this codebase (built-but-unwired) — these two routes are now actively disabled, not just unreferenced. The only user-facing action left on `imotara_wallets` is **refund of an existing balance**, now available regardless of dormant status (the API never actually required it — only the old UI did). `connect_recharges` (the per-consultant system) is unaffected and remains the only real way to pay for a session.
>
> *Original 2026-07-26 entry (kept for history, corrected above): "The `imotara_wallets` top-up UI (web and mobile) has been removed... The backend (`topup/create`/`topup/verify` routes, schema, crons) is untouched and still functions if called... it's just unreachable from any current UI." — the "unreachable" claim was false; see the correction above.*
...
- **Wall-clock reconciliation at completion (added 2026-08-14):** the only ceiling on tick frequency was the 55s floor, so a client ticking right at that boundary could accrue billed minutes ~9% faster than real time over a long session. At completion (paths B and C, plus the `connect-orphans` cron and the `consultant/sessions` stale-session cleanup — all four completion sites), billed minutes are capped to `min(ticked minutes, ceil(wall-clock elapsed since started_at))`; `minutes_used` itself keeps the raw tick count as an audit trail, only the *billed* amount (and what the consultant is credited) is capped. A capped session logs a `wall-clock reconciliation` warning for observability. `amount_charged` is also now uniformly `.toFixed(4)`-rounded at every completion path (previously two of the four paths wrote an unrounded float, and even the DB write is now consistent with the value shown in the completion emails, which previously recomputed its own unrounded figure independently).
