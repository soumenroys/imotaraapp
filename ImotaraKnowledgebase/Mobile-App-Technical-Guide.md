...
- **Launch-flag env-var name mismatch** — `eas.json` sets `EXPO_PUBLIC_LAUNCH_CLOUD_SYNC_FREE_FOR_ALL` (`true` on TestFlight, `false` on production) and RELEASE_GO_NO_GO says to verify it, **but** `HistoryContext.tsx` actually computes the override from `EXPO_PUBLIC_IMOTARA_LAUNCH_DATE` / `EXPO_PUBLIC_IMOTARA_FREE_DAYS` (defaulting to `true` when no launch date is set). Setting the eas.json var alone does not change the code path. A real trap on release day.
...
- **Tests** — the app historically shipped with zero automated tests. A **Jest safety-net branch exists** (`test/safety-net`, commits `d03dade` + `7889964`): 82 tests across 6 suites (`src/__tests__/`: aiClientTone, featureGates, fetchWithTimeout, languageDetection, ttsChunking, versionSync), jest-expo config + native-module stubs (`jest/stubs/`), the version-sync script, and `.github/workflows/ci.yml` (typecheck → version-sync → jest on every push/PR). It also fixes the soft-launch institutional-gate bug (see §4). Until pushed/merged, CI isn't active on the remote.
