...
2. Server checks, in order: age gate (403) → consultant_id is a UUID → rate-limit **max 10 creates / 10 min** (429) → no existing `pending`/`active` session with this consultant (409) → **no `pending`/`active` session with ANY other consultant** (409, "end it first") → consultant is `approved` → **`is_online`** (instant only, else 409 "offline") → not `is_busy` (409) → not blocked by consultant (403) → not your own profile (403) → **`get_session_balance ≥ 1` minute else 402 `needs_recharge`** → **rate check: only a negative/missing `rate_per_min` is rejected** (409 "Consultant rate unavailable") — `rate_per_min = 0` (a companion offering a free session, see A4) is a valid rate and passes.
   - **Fixed 2026-08-16:** this last check used to be `rate <= 0`, which incorrectly rejected every free (`rate_per_min = 0`) companion — free sessions could never actually be started, only advertised. The same falsy-zero flaw also existed in the per-minute billing tick (A7): a free session that somehow got created would have had every tick abort with `session_rate_invalid`, freezing the session. Both are fixed; a companion's `rate_per_min = 0` now works end-to-end (booking, ticking, and the consultant's earnings-history display, which had the same bug on mobile).
...
- **Translate:** `POST /api/connect/translate` `{ text, targetLang, sourceLang? }` — auth-gated (prevents open-proxy abuse), **rate-limited 60 requests / 60s per user**. Four-tier engine selection in `translateText()` (`src/lib/connect/translate.ts`), evaluated in order:
  1. **Romanized-Indic LLM translation** (`gpt-4.1-mini` via `OPENAI_API_KEY`) — used ONLY when `sourceLang` is one of the 9 languages confirmed to translate poorly through Google when romanized (Marathi, Bengali, Tamil, Telugu, Gujarati, Punjabi, Kannada, Malayalam, Odia, Urdu — Hindi excluded, it already works well via Google) AND the text contains no native script for that language (i.e. is genuinely romanized/Latin-script). Added 2026-08-14 after a 37-case evaluation showed Google translating essentially none of these correctly (often not translating at all, or dropping content in code-switched messages) vs. the LLM getting effectively all of them right, across both translation directions, code-switched text, and paragraph-length messages. Hardened against prompt injection — the source text is always treated as content to translate, never as instructions, even if it contains phrases that look like commands.
  2. **Google Cloud Translation** (`GOOGLE_TRANSLATE_API_KEY`) — primary engine for everything else, and the fallback if the LLM call above fails, errors, times out, or returns an empty/non-translation (e.g. echoing the input back unchanged).
  3. **General-purpose LLM translation** (added 2026-08-16) — a non-romanized-specific `gpt-4.1-mini` fallback, tried when Google is unconfigured or errors, for ANY source/target pair (not just the 9 languages in tier 1).
  4. **MyMemory** (free) — absolute last resort, used only if both Google and the general LLM fallback are unavailable.
  ...
  **Production gap (found and fixed 2026-08-16):** despite tier 2 being coded and locally tested back on 2026-08-13, `GOOGLE_TRANSLATE_API_KEY` had never actually been added to the Vercel deployment — so every live Connect translation had silently been running on tier 3/4 (MyMemory) the whole time, which is what surfaced a corrupted MyMemory community entry ("Bhalo" → "kothay tumi") in production. The key is now set in Vercel and tier 3 (general LLM fallback) was added the same day as an extra safety net, so a future env-var gap like this degrades gracefully instead of reaching MyMemory.
