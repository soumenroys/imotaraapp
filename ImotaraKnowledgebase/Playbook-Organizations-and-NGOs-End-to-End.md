...
4. **NGO/EDU Certificate.** `GET /api/org/certificate` (any org **member** can fetch). **Eligibility (all required):** billing type **ngo or edu**, status **active**, **≥10 active members** (`seats_used >= 10`), and **org active ≥30 days**. If eligible it returns a printable **HTML** "Emotional Wellness Champion" certificate; if not, a JSON reason (e.g. "Need at least 10 active members (currently N)"). **Govt/commercial orgs are not eligible.** (v1.2.7 fixed a badge that showed "Eligible" on seat count alone and then silently failed the 30-day check.)
