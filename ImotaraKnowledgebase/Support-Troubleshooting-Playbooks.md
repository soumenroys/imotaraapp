...
4. **Outage check (Gemini path).** If `meta.from:"fallback"`, OpenAI errored. A throttled red-alert email (subject **"🔴 ALERT: OpenAI API unavailable — Gemini fallback active"**, ≤1 per 5 min per instance) goes to `info@imotara.com`. In Vercel logs search `[imotara][aiClient] OpenAI error HTTP` or `[imotara][aiClient] fetch exception`. Confirm https://status.openai.com.
   - **Streaming path (as of 2026-08-14):** both platforms try `?stream=1` first — a streaming SSE response has no `meta.from` field, so an OpenAI failure there won't show up via step 2 above. Look for `[imotara][aiClient] stream fetch exception` or `stream HTTP <code> or missing body` in Vercel logs instead; the same Gemini fallback + outage alert fires. A failure *after* some tokens already streamed does not re-fall-back (would splice two models' output into one reply) — it just ends there, and the client's own stream-stall retry (web: 20s) takes over.
