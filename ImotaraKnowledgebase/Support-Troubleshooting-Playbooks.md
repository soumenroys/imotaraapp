...
1. **Ban alert points to an undocumented mailbox.** The mobile "You've been signed out" alert (`AuthContext.tsx`) tells users to contact **`support@imotara.com`**, but the documented escalation inbox everywhere else (FAQ, crash report `mailto`, AI outage alert) is **`info@imotara.com`**. Banned users appealing via `support@` may reach an unmonitored inbox. *Recommend aligning the alert text to `info@imotara.com` or ensuring `support@` is monitored.*
...
4. **`user_bans` is audit-only; `auth.users.banned_until` is the truth.** A ban applied directly in Supabase (not via the admin route) would leave **no `user_bans` row**, so the ban-status endpoint (`GET /api/admin/users/[userId]/ban`, which reads `user_bans`) could report "not banned" for a genuinely banned user. Always confirm bans against `auth.users.banned_until`.
