...
export async function POST(req: NextRequest) {
  ...
  const { data: consultant, error } = await supabase
    ...
    .insert({
      ...
      preferred_lang:       (typeof preferred_lang === "string" && ["en","hi","bn","mr","ta","te","gu","pa","kn","ml","or","ur","ar","he","ru","zh","ja","es","fr","de","pt","id"].includes(preferred_lang.trim())) ? preferred_lang.trim() : "en",
      status:               "pending",
    })
    .select("id, status")
    .single();
