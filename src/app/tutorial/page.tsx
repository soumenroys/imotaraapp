      ...
      <Phone>
        <div className="p-3 space-y-2">
          <div className="text-center py-3">
            ...
            <p className="text-[9px] text-zinc-500 mt-0.5">Your private companion</p>
          </div>
...
const FEATURES: Record<string, Feature[]> = {
  ...
  chat: [
    ...
    },
    {
      ...
      short: "Each reply is tagged with a detected emotion — tap to confirm or correct it.",
      long: "Below every companion reply, Imotara shows an emotion chip (e.g., '😟 Anxious', '😊 Hopeful') representing the emotional tone it detected in your message. Tapping it logs that emotion to your history, which feeds into your Trends charts. You can also tap a different chip to correct the detection.",
      steps: [
    ...
    {
      icon: "👆",
      title: "Long-Press the Empty Space",
      short: "Hold any blank part of the chat to open the ⋯ menu without reaching for the top bar.",
      long: "On iPhone and Android, pressing and holding an empty area of the conversation — the background around the bubbles, not a bubble itself — opens the same ⋯ overflow menu that lives in the top bar: Breathing exercise, Unsent letter, and Clear chat. It exists for one-handed use, when the top of the screen is out of comfortable thumb reach. Long-pressing a message bubble still does exactly what it always did, so nothing you already rely on changes.",
      steps: [
        "Open the Chat tab.",
        "Press and hold for a moment on empty space between or beside the message bubbles.",
        "The ⋯ menu opens: Breathing exercise, Unsent letter, Clear chat.",
        "Tap an item, or tap outside the menu to dismiss it.",
      ],
      tip: "Mobile only — on the web app, use the ⋯ button in the top bar.",
      badge: "Mobile only",
    },
    {
      icon: "🔖",
    ...
    },
    {
      ...
      long: "After a meaningful conversation, the emotional tone is reflected in your Trends tab — check the mood chart and FeelSection to see what came up. The Trends tab is your reflection space after each session.",
      steps: [
        "Have a multi-message conversation.",
        "Open the Trends tab after your session.",
        "Tap a mood chip in the FeelSection or check your mood chart.",
    ...
    {
      icon: "☁️",
      title: "Cloud vs On-Device Replies",
      short: "Imotara automatically uses cloud or on-device replies — switching is seamless and invisible.",
      long: "Imotara routes each message to either the cloud (richer, context-aware) or the on-device engine (instant, works offline). Switching is automatic — when your daily cloud quota is reached or you're offline, on-device replies take over without any interruption. By default this switch is completely silent and invisible to keep the experience feeling natural. If you'd like to see which engine is used for each reply, you can enable sync status indicators in Settings.",
      steps: [
  ...
  voice: [
    {
      ...
      short: "Tap the microphone to speak your message — Imotara transcribes your words.",
    ...
    },
    {
      ...
      short: "Speak, listen and reply without touching the screen — the mic reopens itself after each answer.",
      long: "With Hands-free on, a whole conversation runs without a single tap: you speak, Imotara transcribes and sends, the reply is read aloud, and the microphone reopens by itself for your next turn. It's built for the moments when your hands or eyes are busy — walking, cooking, lying down — or when reading a screen is simply too much. A banner at the top of Chat always shows the current state (listening, thinking, speaking, or paused), so you are never left guessing whether it can hear you.",
      steps: [
        "Go to Settings → Experience → Voice input.",
        "Turn on 'Hands-free conversation'.",
        "Return to Chat and tap the microphone once to start the first turn.",
        "Speak, then simply stop — Imotara notices the silence and sends.",
        "Listen to the reply; the microphone reopens on its own for your next turn.",
        "To finish, tap the microphone button — after a turn you ended yourself, Imotara will not reopen it.",
      ],
      tip: "Leaving the app and coming back leaves hands-free PAUSED rather than quietly grabbing the microphone — tap the banner or the mic when you're ready to carry on.",
      badge: "Mobile only",
    },
    {
      icon: "🔇",
      title: "Noise Rejection in Hands-Free",
      short: "Traffic, fans and a television in the next room no longer become words you never said.",
      long: "In hands-free mode Imotara measures the sound it is actually picking up instead of assuming everything it hears is speech. A turn that never rises above the noise floor is dropped after about 10 seconds and never uploaded at all. A turn with steady sound but none of the variation speech has — an engine, a fan, a TV next door — stops after about 20 seconds. Only turns that really look like someone talking run on to the full recording limit. On Android the microphone is opened through the phone's speech-recognition audio path, so the same noise suppression your device's own dictation uses is applied. And the transcription service is separately told to throw away its own low-confidence guesses, so a silent room no longer comes back as an invented sentence.",
      steps: [
        "Nothing to switch on — this applies automatically whenever hands-free is running.",
        "When a turn is discarded, the conversation just waits for you; nothing is sent and nothing is charged.",
        "Ordinary tap-to-record voice input is never filtered this way — what you record is always sent.",
      ],
      tip: "The filter is deliberately generous: when in doubt it lets the audio through, because losing a real sentence is worse than occasionally transcribing a noisy one.",
      badge: "Mobile only",
    },
    {
      icon: "⏱",
      title: "Voice Recording Duration Limit",
      short: "Set how long a single voice recording can run — 30 seconds, 1, 2, or 5 minutes.",
      long: "By default, voice recordings stop at 60 seconds. You can change this in Settings under Experience → Voice input. Shorter limits use less data; longer limits let you speak more freely without interruption. The limit is a ceiling, not a target — recording also stops on its own when you go quiet.",
      steps: [
        "Go to Settings and expand 'Experience'.",
        "Scroll to 'Voice input' → 'Max recording duration'.",
        "Tap one of the four options: 30s, 1 min, 2 min, or 5 min.",
        "Changes take effect immediately on the next recording.",
      ],
      tip: "Just below it, 'Recording quality' (High / Low) trades audio detail against upload size — Low is useful on a slow or metered connection.",
    },
    {
      icon: "🌐",
      title: "Cloud Transcription — and What Turning It Off Does",
      short: "On web you can switch to your browser's own speech recognition. On mobile this switch turns voice input off entirely.",
      long: "By default your recording is sent to Imotara's servers for high-quality transcription. The two platforms differ, and the difference matters: on WEB, turning 'Cloud transcription' off falls back to your browser's built-in speech recognition — less accurate, but the audio never leaves your machine. On MOBILE there is no on-device alternative: the setting is called 'Online transcription' and switching it off disables the microphone altogether, which the setting itself says plainly. Turn it off on a phone only if you intend to stop using voice input.",
      steps: [
        "Web: Settings → Appearance → 'Cloud transcription'. Toggle off for browser-only recognition.",
        "Mobile: Settings → Experience → Voice input → 'Online transcription'.",
        "On mobile, leaving it ON is what keeps the microphone working.",
        "On either platform, ON (the default) uses cloud Whisper transcription.",
      ],
      badge: "Cloud transcription requires internet",
      tip: "Whichever platform you're on, the audio is transcribed and then discarded — the server keeps only a usage count, never the recording. On mobile, the temporary copy on your phone is swept within the hour.",
    },
    {
      icon: "✅",
      ...
      long: "By default, after transcription the text goes into the input box and you must manually press Send. You can also enable 'Auto-send on transcription' which sends immediately. Or enable 'Confirm before send' which shows a review modal. Both settings exist on web and mobile.",
      steps: [
        "Web: Settings → Appearance → the voice/recording settings. Mobile: Settings → Experience → Voice input.",
  ...
  companion: [
    {
      ...
      short: "Give your companion a name — it's used everywhere your companion speaks, including notifications.",
      long: "By default your companion is named 'Imotara'. You can rename it anything — a real name, a nickname, or whatever feels right. The name isn't only used inside replies: the chat header, the companion's own prompts and cards, the unsent-letter and grief spaces, the screen-reader labels on each message, and your daily check-in and 'we miss you' notifications all use the name you chose. Imotara stays the name of the app; your companion is whoever you decide it is.",
      steps: [
        "Web: Settings → Tone & Context Preferences. Mobile: Settings → Your companion.",
        "Turn on the companion settings first if they're off (web: the 'Expected companion tone' switch).",
        "Find the 'Companion name (optional)' field and type the name you want.",
        "Web: press Save at the top-right of the card. Mobile: just tap outside the field — it saves by itself.",
        "The new name appears from the next reply onward, and across the rest of the app immediately.",
      ],
      tip: "If you already have a daily check-in reminder switched on, renaming re-issues it — tomorrow's notification greets you with the new name instead of the old one. One honest limit: the 🔊 voice preview speaks your custom name in English only; in other languages it plays a pre-recorded sample that still says 'Imotara'.",
    },
    {
      icon: "👤",
    ...
    },
    {
      ...
      long: "Five response styles shape how the companion replies: 'Comfort me' focuses on emotional validation. 'Help me reflect' asks thought-provoking questions. 'Motivate me' is encouraging and forward-looking. 'Give advice' offers practical guidance. 'Let Imotara decide' — Imotara chooses based on emotional context.",
      steps: [
        "Go to Settings → Your companion → Tone & Context.",
        "Find 'Response style'.",
  ...
  history: [
    ...
    {
      icon: "🔀",
      title: "Sync Status (Optional Indicator)",
      short: "Sync status indicators are hidden by default — enable them in Settings if you want to see them.",
  ...
  trends: [
    ...
    },
    {
      ...
      long: "The Weekly Report is a short paragraph generated by summarising the emotional themes of your week — what you felt, patterns it noticed, and a gentle observation. It appears in the Trends tab and (if enabled) as a push notification.",
      steps: [
  ...
  experience: [
    {
      ...
      long: "On web, Imotara can send you browser push notifications for daily reminders, streak warnings, and weekly emotional summaries. You must grant permission when prompted. If you accidentally blocked them, re-enable in your browser's site settings. On mobile the equivalent lives in Settings → Experience → 'Daily check-in reminder', with its own time, sound and badge controls — and the reminder greets you in your companion's name, not the app's.",
      steps: [
        "Go to Settings → Experience.",
        "Web: find 'Browser notifications' and tap 'Enable notifications'. Mobile: turn on 'Daily check-in reminder'.",
        "Allow the permission prompt when it appears.",
        "Set your preferred reminder time (mobile uses hour and minute steppers; minutes step by 15).",
        "Optional on mobile: turn on 'Play sound' and 'Show badge', and set 'Nudge if silent for'.",
      ],
      tip: "Switching the reminder off turns everything off — including any 'we miss you' nudge already waiting in the queue. And if a reminder can't be set, Imotara now tells you what actually went wrong rather than blaming permissions.",
    },
    {
      icon: "💭",
    ...
    },
    {
      ...
      long: "The Mindset Analysis can look at different time windows: Today, Last 7 days, Last 30 days, and All time. Toggle each on or off to control what data Imotara summarises when generating your capsule. If you want a fresh weekly snapshot only, turn off 'All time'.",
      steps: [
    ...
    },
    {
      ...
      short: "Show or hide the 'Synced' badges and sync indicators on messages — hidden by default.",
  ...
  privacy: [
    {
      ...
      long: "Imotara sends two kinds of email. Essential messages — password resets, sign-in links, changes to the terms — are part of having an account and cannot be switched off, because you need them. Updates and offers are different: we only send those to people who asked, and we check the address really belongs to you before we ever write again. We never buy or scrape address lists. Every update email carries a one-click unsubscribe link, and it takes effect immediately, including for anything already queued to go out.",
      steps: [
        "To start receiving updates: go to imotara.com/updates, enter your address and tick the consent box.",
        "We then send one email asking you to confirm the address. Nothing else follows until you press that link — so if someone types your address by mistake, just ignore it.",
        ...
        "You do not need to sign in, reply, or give a reason, and it applies to anything already on its way.",
  ...
  plans: [
    ...
    },
    {
      ...
      short: "Buy extra reply credits that never expire — use them on top of your daily limit.",
      long: "Token packs are one-time purchases that give you extra cloud replies beyond your daily limit. They never expire and carry over indefinitely. Useful for days when you need to talk more than your plan allows. Available to all tiers including Free.",
      steps: [
  ...
  languages: [
    {
      ...
      long: "Indian languages: English, Hindi, Marathi, Bengali, Tamil, Telugu, Gujarati, Punjabi, Kannada, Malayalam, Odia, Urdu. International: Spanish, French, German, Portuguese, Russian, Arabic, Chinese (Mandarin), Japanese, Hebrew, Indonesian. All are supported in chat, TTS, and on-device replies.",
      steps: [
    ...
    },
    {
      ...
      long: "Code-switching — where you mix two languages in the same sentence — is very common in multilingual communities. Imotara understands code-switched text and responds in kind, typically matching the primary language of the message.",
      steps: [
